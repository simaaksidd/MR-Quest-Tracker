...
def calculate_potential_points(quests, teammates, location, enemies):
    ...
    for quest in quests:
        ...
        quest_enemy = quest["enemy"]
        points = quest["reward"]
        questline = quest["questline"]
        
        #idk how to simplify ts
        #Add all heroes with no 'conditions' 
        if not partner and not quest_location and not quest_enemy and hero not in teammates:
            hero_scores[hero]["points"] += points
            hero_scores[hero]["quests"].append(questline)

        if partner in teammates:
            hero_scores[hero]["points"] += points
            hero_scores[hero]["quests"].append(questline)
